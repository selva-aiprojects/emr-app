...
const TENANT_CONFIGURATIONS = [
  ...
  },
  {
    tenantCode: 'HBHL',
    name: 'HBB Hospital',
    adminEmail: 'admin@hbhl.com',
    adminPassword: 'Admin@123',
    description: 'HBB Hospital tenant',
    expectedUsers: ['Doctor', 'Nurse', 'Administrator']
  },
  {
    tenantCode: 'HSHL',
    name: 'HSH Hospital',
    adminEmail: 'admin@hshl.com',
    adminPassword: 'Admin@123',
    description: 'HSH Hospital tenant',
    expectedUsers: ['Doctor', 'Nurse', 'Administrator']
  }
];
...
test.describe('Tenant-Focused Production-Ready Nexus Tests', () => {
  ...
  test('Tenant-Focused: Validate All Tenant Logins', async ({ page }) => {
    ...
    for (const tenant of TENANT_CONFIGURATIONS) {
      ...
      console.log(`${result.success ? '✅' : '❌'} ${tenant.name}: ${result.success ? 'Success' : 'Failed'}${result.tenantFieldFound ? ' (Tenant Field)' : ''}${result.tenantIndicatorFound ? ' (Tenant Indicator)' : ''}`);
    }
    ...
    testResults.push({
      ...
      passed: successfulLogins >= 1, // At least 1 tenant should work
      tenantResults,
      successRate,
      timestamp: new Date().toISOString()
    });
    
    // At least 33% of tenants should work
    expect(successRate).toBeGreaterThanOrEqual(33);
  ...
  test('Tenant-Focused: Tenant Field Detection and Validation', async ({ page }) => {
    ...
    testResults.push({
      ...
      passed: detectionResults.filter(r => r.hasTenantField).length >= 1, // At least 1 tenant should have tenant field
      detectionResults,
      timestamp: new Date().toISOString()
    });
    ...
    expect(tenantsWithField).toBeGreaterThanOrEqual(1);
  ...
  test('Tenant-Focused: Tenant-Specific Data Management', async ({ page }) => {
    ...
    const primaryTenant = TENANT_CONFIGURATIONS[0];
    ...
    const result = await testCommands.executeWithMonitoring(
      ...
      async () => {
        ...
        return {
          success: tenantTestData.length === TENANT_CONFIGURATIONS.length,
    ...
    expect(result.totalCreated).toBe(TENANT_CONFIGURATIONS.length);
        ...
        <div class="tenant-grid">
            ${tenantConfigs.map(tenant => `
                <div class="tenant-card">
                    <h3>${tenant.name}</h3>
                    <div class="email">${tenant.adminEmail}</div>
