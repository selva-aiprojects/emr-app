...
<<<<<<< HEAD
    ...
    const isChartVisible = await echartsCanvas.isVisible({ timeout: 10000 }).catch(() => false);
    
    console.log('Chart canvas visible: ' + isChartVisible);
    
    if (isChartVisible) {
      const boundingBox = await echartsCanvas.boundingBox({ timeout: 5000 });
      console.log(`Patient Overview Chart size: ${boundingBox ? boundingBox.width + 'x' + boundingBox.height : 'no bbox'}px`);
      if (boundingBox) {
      ...
      // Fallback: page loaded with content
...
<<<<<<< HEAD
    ...
    const hasRevenueText = /revenue|finance|income|billing/i.test(pageText) || pageText.includes('Dashboard') || pageText.includes('Analytics');
