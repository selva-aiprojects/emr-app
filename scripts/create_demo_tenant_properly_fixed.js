...
async function createDemoTenantProperly() {
  ...
  try {
    ...
    if (existingTenant.rows.length > 0) {
      ...
      await updateTenantWithProperSchema(tenant);
    } else {
      console.log(' Creating new DEMO tenant with proper architecture...');
      const tenant = await createNewTenantWithProperSchema();
      await setupTenantSchema(tenant);
      await seedInitialData(tenant);
    }
...
async function createNewTenantWithProperSchema() {
  ...
  // Step 4: Add schema_name column if it doesn't exist
  try {
...
async function updateTenantWithProperSchema(tenant) {
  ...
  if (tableCheck.rows.length < 50) {
    ...
    await migrateExistingDataToTenantSchema(tenant);
  }
...
async function seedInitialData(tenant) {
  console.log(' Seeding initial data for tenant...');
  ...
  try {
    ...
    await query(`
      INSERT INTO ${schemaName}.wards (tenant_id, name, type, base_rate, status)
      SELECT $1, 'General Medicine Ward', 'General', 1500, 'Active'
      WHERE NOT EXISTS (SELECT 1 FROM ${schemaName}.wards WHERE tenant_id = $1)
    `, [tenantId]);

    const wardId = await query(`SELECT id FROM ${schemaName}.wards WHERE tenant_id = $1 AND name = 'General Medicine Ward' LIMIT 1`);

    if (wardId.rows.length > 0) {
      await query(`
        INSERT INTO ${schemaName}.beds (tenant_id, ward_id, bed_number, type, status)
        SELECT $1, $2, 'Unit-01', 'General', 'Available'
        WHERE NOT EXISTS (SELECT 1 FROM ${schemaName}.beds WHERE tenant_id = $1)
      `, [tenantId, wardId.rows[0].id]);
    }

    // Create basic departments
    const departments = [
      { name: 'General Medicine', code: 'GM', hod_user_id: null },
      { name: 'Cardiology', code: 'CARD', hod_user_id: null },
      { name: 'Pediatrics', code: 'PED', hod_user_id: null },
      { name: 'Laboratory', code: 'LAB', hod_user_id: null },
      { name: 'Pharmacy', code: 'PHARM', hod_user_id: null },
      { name: 'Billing', code: 'BILL', hod_user_id: null },
      { name: 'Administration', code: 'ADMIN', hod_user_id: null }
    ];

    for (const dept of departments) {
      try {
        await query(`
          INSERT INTO ${schemaName}.departments (tenant_id, name, code, hod_user_id, status, created_by, created_at, updated_at)
          VALUES ($1, $2, $3, $4, 'active', NULL, NOW(), NOW())
        `, [tenantId, dept.name, dept.code, dept.hod_user_id]);
      } catch (error) {
        console.log(` Department ${dept.name} already exists or failed: ${error.message}`);
      }
    }
    ...
    const employees = [
      ...
      { name: 'Nurse Anita Desai', code: 'NUR001', email: 'anita@demo.hospital', phone: '+91-9876543203', department: 'General Medicine', designation: 'Head Nurse', salary: 60000 },
      { name: 'Meera Reddy', code: 'PHM001', email: 'meera@demo.hospital', phone: '+91-9876543205', department: 'Pharmacy', designation: 'Pharmacist', salary: 55000 }
    ];
    ...
    for (const emp of employees) {
      try {
        ...
        `, [
          tenantId, emp.code, emp.name, emp.email, emp.phone, emp.department, emp.designation, emp.salary, emp.salary, 'Day', 12, true, emp.join_date || new Date().toISOString().split('T')[0], emp.leave_balance || 12, true, new Date(), new Date(), 'ACC' + Math.floor(Math.random() * 9000 + 1000)
        ]);
      } catch (error) {
        console.log(` Employee ${emp.name} already exists or failed: ${error.message}`);
      }
    }
    ...
    console.log(' Initial data seeding skipped or failed:', error.message);
  }
}

async function migrateExistingDataToTenantSchema(tenant) {
    ...
    for (const item of inventoryItems.rows) {
      try {
        ...
        `, [
          ...
          item.current_stock, item.reorder_level, item.unit, item.unit_price, 
          item.expiry_date, item.created_at, item.updated_at
        ]);
      } catch (error) {
        // Ignore duplicates
      }
