...
async function populateDemoTenantFinalFixed() {
  ...
  for (let i = 0; i < 100; i++) {
    ...
    try {
      ...
      `, [
        ...
        new Date(),
        getRandomItem(['available', 'reserved', 'issued', 'discarded', 'expired', 'discarded']),
    ...
    for (let i = 0; i < 730; i++) {
      ...
      try {
        ...
        `, [
          ...
          isPresent ? new Date(attendanceDate.getTime() + getRandomInt(16, 18) * 60 * 1000) : null,
  ...
  for (let i = 0; i < 150; i++) {
    ...
    try {
      ...
      `, [
        ...
        getRandomItem(['Hypertension', 'Diabetes Type 2', 'Asthma', 'Arthritis', 'Migraine', 'Depression', 'Depression']),
...
populateDemoTenantFinalFixed().then(result => {
