        ...
        for (let j = 0; j < 2; j++) {
            ...
            await client.query(
                `INSERT INTO emr.prescription_items 
                 (prescription_id, drug_name, sequence, dose, dose_unit, frequency, route, quantity_prescribed, status, instructions, tenant_id) 
                 VALUES ($1, $2, $3, 1, 'tablet', 'BID', 'Oral', 10, 'pending', 'E2E test item ${j+1}', $4)`,
                [pxId, drugName, j + 1, tenantId]
            );
            await client.query(
                `INSERT INTO emr.prescription_items 
                 (prescription_id, sequence, dose, dose_unit, frequency, route, quantity_prescribed, status, instructions, tenant_id) 
