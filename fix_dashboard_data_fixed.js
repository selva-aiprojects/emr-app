...
async function fixDashboardData() {
  try {
    ...
    console.log('=== FIXING DASHBOARD DATA (CORRECTED SCHEMA) ===\n');
    
    // Set search path to demo_emr schema (CORRECT APPROACH)
    ...
    try {
      ...
      if (wards.rows.length === 0) {
        ...
        for (const wardType of wardTypes) {
          const wardResult = await query(`
            INSERT INTO wards (tenant_id, name, type, base_rate, status, created_at, updated_at)
      ...
      // Get wards again
      const updatedWards = await query('SELECT id, name, type FROM wards WHERE tenant_id = $1', [tenantId]);
    ...
    try {
      await query('SELECT COUNT(*) FROM blood_units WHERE tenant_id = $1', [tenantId]);
      ...
      for (let i = 0; i < 30; i++) {
        try {
          ...
          await query(`
            ...
            ON CONFLICT DO NOTHING
          `, [tenantId, getRandomItem(bloodGroups), collectionDate.toISOString().split('T')[0], 
      ...
      const bloodUnitCount = await query('SELECT COUNT(*) as count FROM blood_units WHERE tenant_id = $1 AND status = \'available\'', [tenantId]);
    ...
    try {
      await query('SELECT COUNT(*) FROM discharges WHERE tenant_id = $1', [tenantId]);
    ...
    // 7. Create Today's Attendance
    console.log('\n7. Creating Today\'s Attendance...');
    
    try {
      await query('SELECT COUNT(*) FROM attendance WHERE tenant_id = $1', [tenantId]);
      
      const employees = await query('SELECT id FROM employees WHERE tenant_id = $1', [tenantId]);
      ...
      for (const employee of employees.rows) {
        try {
          ...
          await query(`
            INSERT INTO attendance (tenant_id, employee_id, date, check_in, check_out, status, created_at, updated_at)
            VALUES ($1, $2, $3, $4, $5, $6, NOW(), NOW())
            ON CONFLICT (tenant_id, employee_id, date) DO UPDATE SET updated_at = NOW()
          `, [tenantId, employee.id, today, present ? '09:00:00' : null, present ? '17:00:00' : null, present ? 'present' : 'absent']);
        } catch (error) {
          // Ignore duplicates
        }
      }
      
      const attendanceCount = await query('SELECT COUNT(*) as count FROM attendance WHERE tenant_id = $1 AND date = CURRENT_DATE AND status = \'present\'', [tenantId]);
      console.log(`Today's Attendance: ${attendanceCount.rows[0].count}`);
    ...
    console.log('\nAll dashboard cards should now show complete data!');
