...
(async () => {
  try {
    ...
    console.log('✅ Basic connection:', test.rows[0]);
    ...
    // Check tenant tables
    const tenants = await query('SELECT COUNT(*) as count FROM nexus.tenants');
    console.log('📊 Tenants count:', tenants.rows[0].count);
    
    const mgmtTenants = await query('SELECT COUNT(*) as count FROM nexus.management_tenants');
