  ...
  try {
    ...
    for (let i = 0; i < 90; i++) {
      ...
      const appointmentData = {
        ...
        provider_id: crypto.randomUUID(), // Random UUID for provider
    ...
    for (let i = 0; i < 70; i++) {
      ...
      const invoiceData = {
        ...
        tax: amount * 0.1,
        total: amount,
        paid: invoiceStatuses[Math.floor(Math.random() * invoiceStatuses.length)] === 'paid' ? amount : 0,
    ...
    for (let i = 0; i < 15; i++) {
      const bedData = {
        ...
        ward_id: crypto.randomUUID(), // Random UUID for ward
    ...
    for (let i = 0; i < 50; i++) {
      ...
      const serviceData = {
        ...
        requested_by: crypto.randomUUID(), // Random UUID for requester
        created_at: randomDate,
        updated_at: randomDate
      };
