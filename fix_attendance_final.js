...
async function fixAttendanceFinal() {
  try {
    const tenantId = '20d07615-8de9-49b4-9929-ec565197e6f4';
    
    console.log('=== FIXING TODAY\'S ATTENDANCE ===\n');
    
    // Check current attendance
    const currentAttendance = await query('SELECT COUNT(*) as count FROM demo_emr.attendance WHERE tenant_id = $1 AND date = CURRENT_DATE AND status = \'present\'', [tenantId]);
    console.log(`Current Today's Attendance: ${currentAttendance.rows[0].count}`);
