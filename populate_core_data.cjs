...
(async () => {
  try {
    console.log('=== Populating Starlight Dashboard with Core Monthly Data ===');
    ...
    for (let i = 0; i < 25; i++) {
      const bedData = {
        ...
        patient_id: bedStatuses[Math.floor(Math.random() * bedStatuses.length)] === 'occupied' ? allPatientIds[Math.floor(Math.random() * allPatientIds.length)] : null,
        created_at: new Date(),
        updated_at: new Date()
      };
    ...
    // 5. Add more encounters (total 80 encounters)
    console.log('\n=== Adding 50 more encounters ===');
    ...
    console.log('\n=== Core Monthly Data Population Complete! ===');
