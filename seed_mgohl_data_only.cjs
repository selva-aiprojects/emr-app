...
async function seedMgohlDataOnly() {
  console.log("=== SEEDING 2 YEARS OF DATA FOR EXISTING MGHPL ===");
  try {
    ...
    // Ensure MGHPL points to the correct schema
    ...
    for (const file of sqlFiles) {
      ...
      if (fs.existsSync(filePath)) {
        ...
        // Remove any tenant creation/insertion statements to avoid conflicts
        sql = sql.replace(/INSERT INTO emr\.tenants[^;]*;/gi, '');
        sql = sql.replace(/INSERT INTO tenants[^;]*;/gi, '');
