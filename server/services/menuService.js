...
class MenuService {
  ...
  async getMenuItems(headerId, roleName, tenantId = null, subscriptionPlan = null) {
    ...
    if (subscriptionPlan) {
      ...
      query += ` AND NOT mi.requires_subscription`;
    }
    
    query += ` ORDER BY mi.code, (mi.tenant_id IS NOT NULL) DESC, mi.sort_order ASC`;
  ...
  async upsertMenuHeader(headerData) {
    ...
    if (id) {
      // Update existing
      const query = `
        UPDATE emr.menu_header 
        SET name = $1, code = $2, description = $3, sort_order = $4, 
            icon_name = $5, is_active = $6, tenant_id = $7, updated_at = NOW()
        WHERE id = $8
        RETURNING *
      `;
      const result = await pool.query(query, [name, code, description, sort_order, icon_name, is_active, tenant_id, id]);
      return result.rows[0];
    } else {
      // Create new
      const query = `
        INSERT INTO emr.menu_header (name, code, description, sort_order, icon_name, is_active, tenant_id)
        VALUES ($1, $2, $3, $4, $5, $6, $7)
        RETURNING *
      `;
      const result = await pool.query(query, [name, code, description, sort_order, icon_name, is_active, tenant_id]);
      return result.rows[0];
    }
  }
  ...
  async updateRoleMenuAccess(roleName, menuItemId, isVisible, tenantId = null) {
    ...
    const result = await pool.query(query, [roleName, menuItemId, isVisible, tenantId]);
    return result.rows[0];
  }
  ...
  async deleteMenuHeader(headerId) {
    const query = 'DELETE FROM emr.menu_header WHERE id = $1 RETURNING *';
    const result = await pool.query(query, [headerId]);
    return result.rows[0];
  }
  ...
  async deleteMenuItem(itemId) {
    const query = 'DELETE FROM emr.menu_item WHERE id = $1 RETURNING *';
