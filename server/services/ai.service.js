...
export async function generatePatientSummary(tenantId, patientId) {
  try {
    ...
    // Map database snake_case to what the prompt logic expects
    const patientData = {
      ...
      caseHistory: patient.case_history || patient.caseHistory || [],
      medications: patient.medications || [],
      prescriptions: patient.prescriptions || [],
      testReports: patient.test_reports || patient.testReports || []
    };
    ...
    const prompt = `
      ...
      Patient: ${patientData.firstName} ${patientData.lastName} (Age: ${calculateAge(patientData.dob)})
      Medical History: ${JSON.stringify(patientData.medicalHistory || 'None provided')}
      Case History: ${JSON.stringify(patientData.caseHistory)}
      Active Medications: ${JSON.stringify(patientData.medications)}
      Prescriptions: ${JSON.stringify(patientData.prescriptions)}
      Lab Results: ${JSON.stringify(patientData.testReports)}

      Format your response with the following sections:
      1. Clinical Snapshot (Concise Summary highlighting connections between historical data and recent findings)
      2. Key Historical Context (Findings from clinical history)
    ...
    if (!GEMINI_API_KEY) {
      ...
      return mockSummary(patientData);
    }
    ...
    // Return a graceful fallback instead of throwing to the UI
    return "Clinical summary currently unavailable. Please review patient history manually.";
  }
}
...
export async function suggestTreatmentPlan(tenantId, visitDetails) {
  try {
    ...
    return "Unable to generate treatment suggestions at this time. Please follow standard clinical protocols.";
  }
}
...
export async function generateDischargeSummary(tenantId, encounterId) {
  try {
    ...
    const prompt = `
      ...
      Clinical Assessment: ${encounter.assessment || 'None recorded'}
      Treatment Plan: ${encounter.plan || 'None recorded'}
      Clinical Notes: ${encounter.notes}
      Vitals on Stay: 
        BP: ${encounter.vitals?.bp || 'Not recorded'}
        HR: ${encounter.vitals?.hr || 'Not recorded'}
        Temp: ${encounter.vitals?.temperature || 'Not recorded'}
        O2 Sat: ${encounter.vitals?.oxygen_saturation || 'Not recorded'}
      Medications Prescribed: ${JSON.stringify(patient.medical_history?.prescriptions || [])}
    ...
    return "Formal discharge summary generation failed. Please use the manual discharge template.";
  }
}
