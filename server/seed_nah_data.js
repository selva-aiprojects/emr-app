...
async function seedNAHData() {
  try {
    ...
    const verificationQueries = [
      ...
      'SELECT COUNT(*) as count FROM emr.users WHERE tenant_id::text = (SELECT id::text FROM emr.tenants WHERE code = \'NAH\')',
      'SELECT COUNT(*) as count FROM emr.patients WHERE tenant_id::text = (SELECT id::text FROM emr.tenants WHERE code = \'NAH\')',
      'SELECT COUNT(*) as count FROM emr.appointments WHERE tenant_id::text = (SELECT id::text FROM emr.tenants WHERE code = \'NAH\')',
      'SELECT COUNT(*) as count FROM emr.invoices WHERE tenant_id::text = (SELECT id::text FROM emr.tenants WHERE code = \'NAH\')'
