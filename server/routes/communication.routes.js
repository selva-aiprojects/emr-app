...
router.post('/notices', requireRole('Admin', 'Management', 'HR'), async (req, res) => {
  try {
    ...
    const created = await query(
      `INSERT INTO notices (tenant_id, title, body, audience_roles, audience_departments, starts_at, ends_at, status, priority, created_by)
...
router.patch('/notices/:id/status', requireRole('Admin', 'Management', 'HR'), async (req, res) => {
  try {
    ...
    const updated = await query(
      `UPDATE notices SET status = $1, updated_at = NOW() WHERE id = $2 AND tenant_id = $3 RETURNING *`,
