...
router.post('/', requirePermission('emr'), async (req, res) => {
  try {
    const { 
      patientId, providerId, type, complaint, chiefComplaint, diagnosis, assessment, plan, notes, wardId, bedId,
      bp, hr, temperature, oxygen_saturation 
    } = req.body;
    
    const finalComplaint = complaint || chiefComplaint;
    ...
    const isValid = validTypes.some(t => t.toUpperCase() === typeUpper);
