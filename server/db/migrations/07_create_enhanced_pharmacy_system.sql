...
DROP TABLE IF EXISTS pharmacy_inventory_enhanced CASCADE;
DROP TABLE IF EXISTS prescriptions_enhanced CASCADE;
DROP TABLE IF EXISTS prescription_items_enhanced CASCADE;
...
CREATE TABLE pharmacy_inventory_enhanced (
    id VARCHAR(255) PRIMARY KEY DEFAULT gen_random_uuid()::text,
    tenant_id VARCHAR(255) NOT NULL REFERENCES nexus.tenants(id),
    drug_id VARCHAR(255) NOT NULL REFERENCES drug_master(id),
...
CREATE TABLE prescription_items_enhanced (
    id VARCHAR(255) PRIMARY KEY DEFAULT gen_random_uuid()::text,
    prescription_id VARCHAR(255) NOT NULL REFERENCES prescriptions_enhanced(id),
    drug_id VARCHAR(255) NOT NULL REFERENCES drug_master(id),
    drug_name TEXT,
    dosage VARCHAR(64),
...
CREATE TABLE medication_dispensing (
    ...
    prescription_id VARCHAR(255) NOT NULL REFERENCES prescriptions_enhanced(id),
    drug_id VARCHAR(255) NOT NULL REFERENCES drug_master(id),
    batch_id VARCHAR(255) NOT NULL REFERENCES pharmacy_inventory_enhanced(id),
...
INSERT INTO drug_master (tenant_id, name, generic_name, brand_names, dosage_form, strength, category)
...
INSERT INTO pharmacy_inventory_enhanced (tenant_id, drug_id, batch_number, expiry_date, quantity_received, quantity_remaining, unit_cost, unit_price, status)
SELECT dm.tenant_id, dm.id, 'BATCH-001', '2027-12-31', 5000, 5000, 1.50, 5.00, 'ACTIVE'
FROM drug_master dm 
JOIN nexus.tenants t ON dm.tenant_id = t.id
