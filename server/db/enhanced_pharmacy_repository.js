...
export async function createEnhancedPrescription({
  ...
  const sql = `
    INSERT INTO prescriptions_enhanced (
...
export async function checkDrugInteractions(tenantId, drugIds) {
  const sql = `
    ...
    JOIN drug_master dm1 ON di.drug_1_id = dm1.id
    JOIN drug_master dm2 ON di.drug_2_id = dm2.id
...
export async function updateInventoryStock(dispensingLogId, drugId, batchNumber, quantity, movementType) {
  ...
  const inventoryResult = await query(`
    SELECT id, current_stock FROM pharmacy_inventory_enhanced
...
export async function getPharmacyDashboard(tenantId) {
  const sql = `
    SELECT 
      ...
      (SELECT COUNT(*) FROM pharmacy_inventory_enhanced WHERE tenant_id = $1 AND status = 'ACTIVE') as total_inventory_items,
      (SELECT COUNT(*) FROM pharmacy_inventory_enhanced WHERE tenant_id = $1 AND current_stock <= minimum_stock_level AND status = 'ACTIVE') as critical_stock_items,
      (SELECT COUNT(*) FROM pharmacy_inventory_enhanced WHERE tenant_id = $1 AND current_stock <= reorder_level AND current_stock > minimum_stock_level AND status = 'ACTIVE') as low_stock_items,
      ...
      (SELECT COUNT(*) FROM prescriptions_enhanced WHERE tenant_id = $1 AND status = 'ACTIVE') as active_prescriptions,
      (SELECT COUNT(*) FROM prescriptions_enhanced WHERE tenant_id = $1 AND status = 'COMPLETED') as completed_prescriptions,
      ...
      (SELECT COUNT(*) FROM pharmacy_dispensing_log WHERE tenant_id = $1 AND DATE(dispensing_date) = CURRENT_DATE) as today_dispensing,
      (SELECT COALESCE(SUM(net_amount), 0) FROM pharmacy_dispensing_log WHERE tenant_id = $1 AND DATE(dispensing_date) = CURRENT_DATE) as today_revenue,
      (SELECT COUNT(*) FROM pharmacy_dispensing_log WHERE tenant_id = $1 AND emergency_dispensing = true) as emergency_dispensing,
      
      -- Financial Summary
      (SELECT COALESCE(SUM(net_amount), 0) FROM pharmacy_dispensing_log WHERE tenant_id = $1 AND DATE_TRUNC('month', dispensing_date) = DATE_TRUNC('month', CURRENT_DATE)) as monthly_revenue,
      (SELECT COALESCE(SUM(total_amount), 0) FROM pharmacy_dispensing_log WHERE tenant_id = $1 AND DATE_TRUNC('month', dispensing_date) = DATE_TRUNC('month', CURRENT_DATE)) as monthly_gross_revenue,
      
      -- Drug Categories
      (SELECT COUNT(DISTINCT dm.therapeutic_category) FROM pharmacy_inventory_enhanced pie JOIN drug_master dm ON pie.drug_id = dm.id WHERE pie.tenant_id = $1 AND pie.status = 'ACTIVE') as therapeutic_categories,
      (SELECT COUNT(*) FROM pharmacy_inventory_enhanced pie JOIN drug_master dm ON pie.drug_id = dm.id WHERE pie.tenant_id = $1 AND dm.is_narcotic = true AND pie.status = 'ACTIVE') as narcotic_drugs,
      (SELECT COUNT(*) FROM pharmacy_inventory_enhanced pie JOIN drug_master dm ON pie.drug_id = dm.id WHERE pie.tenant_id = $1 AND dm.is_psychotropic = true AND pie.status = 'ACTIVE') as psychotropic_drugs
  `;
