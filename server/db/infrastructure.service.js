...
export async function getDepartments(tenantId) {
  const sql = 'SELECT * FROM departments WHERE tenant_id::text = $1::text ORDER BY name';
...
export async function getWards(tenantId) {
  const sql = 'SELECT * FROM wards WHERE tenant_id::text = $1::text ORDER BY name';
  const result = await query(sql, [tenantId]);
  return result.rows;
}
...
export async function getBeds(tenantId, wardId = null) {
  let sql, params;
  if (wardId) {
    sql = 'SELECT * FROM beds WHERE tenant_id::text = $1::text AND ward_id::text = $2::text ORDER BY bed_number';
    params = [tenantId, wardId];
  } else {
    sql = 'SELECT * FROM beds WHERE tenant_id::text = $1::text ORDER BY bed_number';
    params = [tenantId];
  }
...
export async function getServices(tenantId) {
  const sql = 'SELECT * FROM services WHERE tenant_id::text = $1::text ORDER BY category, name';
  const result = await query(sql, [tenantId]);
  return result.rows;
}
