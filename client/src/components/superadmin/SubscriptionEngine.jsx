...
export default function SubscriptionEngine({ tenants = [] }) {
  ...
  // Calculate Actuals per Tier
  const tierCounts = tenants.reduce((acc, t) => {
    // Note: Since real DB might not have 'tier' in management_tenant_metrics yet, 
    // we use a logic to fallback or derive for "Actuals" look.
  ...
  const handleManualEmail = (type) => {
    showToast({ 
      message: `System triggering ${type} protocol to targeted tenant email...`, 
      type: 'info',
    ...
    <div className="space-y-12 animate-fade-in px-2">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
         ...
         <div className="flex gap-2">
            <button className="flex items-center gap-2 px-6 py-3 bg-white text-slate-900 rounded-2xl text-[10px] font-black uppercase tracking-widest border border-slate-200 shadow-sm hover:bg-slate-50 transition-colors">
               <Edit2 size={12} /> Pricing Update
            </button>
      ...
      {/* Feature Dashboard Matrix with Colorful Actuals */}
         ...
         {PLAN_MODELS.map((plan) => (
            <div 
               ...
               className={`relative overflow-hidden rounded-[40px] p-0.5 transition-all cursor-pointer group ${
                  selectedPlan === plan.id ? 'shadow-2xl shadow-slate-200 -translate-y-2' : 'hover:-translate-y-1'
               }`}
            >
               {/* Border Gradient */}
               <div className={`absolute inset-0 bg-gradient-to-br from-${plan.color}-400 to-${plan.id === 'free' ? 'slate' : plan.id === 'basic' ? 'blue' : plan.id === 'professional' ? 'teal' : 'pink'}-500 ${selectedPlan === plan.id ? 'opacity-100' : 'opacity-10'} transition-opacity`} />
               
               <div className="relative bg-white rounded-[39px] p-8 h-full">
                  <div className={`w-14 h-14 rounded-2xl bg-gradient-to-br from-${plan.color}-500 to-${plan.id === 'free' ? 'slate' : 'blue'}-600 text-white flex items-center justify-center mb-6 shadow-lg shadow-${plan.color}-200 group-hover:scale-110 transition-transform`}>
                     <plan.icon size={28} />
                  </div>
                  ...
                  <p className={`text-sm font-black text-${plan.color}-600 mb-6 uppercase tracking-widest`}>{plan.cost}</p>
                  ...
                  <div className="space-y-4">
                     {plan.features.map((feat, i) => (
                        <div key={i} className="flex items-center gap-3 text-slate-400 font-bold uppercase tracking-widest text-[9px]">
                           ...
                           <span className="group-hover:text-slate-600 transition-colors">{feat}</span>
                        </div>
                     ))}
                  </div>

                  {selectedPlan === plan.id && (
                     <div className={`absolute -bottom-10 -right-10 w-24 h-24 bg-${plan.color}-500/10 blur-3xl rounded-full`} />
                  )}
               </div>
            </div>
         ))}
      ...
      {/* Integration: Communication Linkage */}
      <section className="grid grid-cols-1 xl:grid-cols-2 gap-8">
            ...
            <div className="space-y-4">
               {[
                  ...
                  <div key={idx} className="flex items-center justify-between p-6 bg-white/5 rounded-3xl border border-white/5 hover:bg-white/10 transition-colors group cursor-pointer">
                     ...
                     <button 
                        onClick={(e) => { e.stopPropagation(); handleManualEmail(comm.title); }}
         ...
         <div className="bg-emerald-900 rounded-[40px] p-10 text-white flex flex-col justify-between overflow-hidden relative group">
            <div className="relative z-10">
               <div className="flex items-center gap-3 mb-4">
                  <div className="w-10 h-10 rounded-xl bg-white/10 flex items-center justify-center text-emerald-400">
                     <Zap size={20} />
                  </div>
                  <h3 className="text-sm font-black text-emerald-300 uppercase tracking-widest leading-none">Global Tier Update Matrix</h3>
               </div>
               <p className="text-3xl font-black tracking-tight leading-none mb-4">Active Deployments: 12</p>
               <p className="text-xs text-white/50 font-medium leading-relaxed max-w-[320px]">
                  Adjusting the <span className="text-white font-black">{selectedPlan} shard</span> will affect pricing and capabilities for all connected hospital nodes instantly.
               </p>
            </div>

            <div className="relative z-10 space-y-4 pt-10">
               <div className="flex gap-4">
                  <button className="flex-1 py-4 bg-white text-emerald-900 rounded-2xl text-[11px] font-black uppercase tracking-widest hover:bg-emerald-50 transition-colors shadow-xl">Deploy Updated Logic</button>
                  <button className="px-6 py-4 bg-emerald-700/50 hover:bg-emerald-700 text-white rounded-2xl transition-colors border border-emerald-600/30">
                     <AlertCircle size={20} />
                  </button>
               </div>
               <p className="text-[9px] font-black uppercase tracking-widest text-emerald-400 text-center">* All updates are cryptographically signed by Root Admin</p>
            </div>
