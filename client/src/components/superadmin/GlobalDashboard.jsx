...
export default function GlobalDashboard({ tenants = [], overview = {} }) {
  ...
  const [fixedOverview, setFixedOverview] = React.useState(null);
  const totals = fixedOverview?.totals || overview?.totals || {};

  // Fetch fixed overview data on mount
  React.useEffect(() => {
    const fetchFixedOverview = async () => {
      try {
        const data = await superadminService.getFixedOverview();
        setFixedOverview(data);
      } catch (error) {
        console.warn('Failed to fetch fixed overview:', error.message);
      }
    };
